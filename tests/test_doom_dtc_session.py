...
import world_mcp
from world_mcp import GAMES, DoomDtcSession, _VIZDOOM_WORLDS, _load_pinned_seeds, _static_tools
...
# ---------------------------------------------------------------------------
# _load_pinned_seeds: --seeds-file accepts EITHER one-int-per-line OR a JSON array (GATE-3D PR-D adds
...
def test_load_pinned_seeds_from_json_array_file(tmp_path):
    p = tmp_path / "seeds.json"
    p.write_text("[1000, 1001, 1002]", encoding="utf-8")
    assert _load_pinned_seeds(_ns(seeds_file=str(p))) == [1000, 1001, 1002]


def test_load_pinned_seeds_from_one_int_per_line_file(tmp_path):
    p = tmp_path / "seeds.txt"
    p.write_text("1000\n1001\n1002\n", encoding="utf-8")
    assert _load_pinned_seeds(_ns(seeds_file=str(p))) == [1000, 1001, 1002]


def test_load_pinned_seeds_json_array_takes_priority_over_seed_flag(tmp_path):
    p = tmp_path / "seeds.json"
    p.write_text("[5, 6, 7]", encoding="utf-8")
    assert _load_pinned_seeds(_ns(seeds_file=str(p), seed=[1, 2, 3])) == [5, 6, 7]


def test_load_pinned_seeds_falls_back_to_seed_flag_when_no_seeds_file():
    assert _load_pinned_seeds(_ns(seed=[3, 1, 4])) == [3, 1, 4]


def test_load_pinned_seeds_empty_when_neither_given():
    assert _load_pinned_seeds(_ns()) == []
