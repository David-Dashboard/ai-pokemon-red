...
| `static_objects.py` | what static salient objects sit on this tile-grid | `StaticObjectDetector:146`, `detect:160` | **none in the live tree** (verified: only its own file + tests import it) | **GATE VERDICT: KILL CHEAP** — recall 0.0, precision 0.0, **154 phantoms** (`HANDOFF.md:456`, live-reproduced 2026-07-05 by re-running the scorer; the docstring's own "236-341" (:19-22) does NOT reproduce — trust the run, not the comment). Kept as a documented-honest failure; do not resurrect without a new realizer and a fresh measured bar |
