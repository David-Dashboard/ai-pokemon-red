...
| `planned_model` | David's decision 2026-07-21 + fresh independent observation | `gpt-5.6-sol` (OpenAI "Sol"): (a) David's explicit decision 2026-07-21 -- the design-doc-era `gpt-5.4` plan was stale; (b) independently observed 2026-07-21 via `codex doctor` on the launch machine (codex-cli 0.144.3), Configuration section line `model                    gpt-5.6-sol · openai`, auth `stored auth mode chatgpt` -- not read from any run receipt. No `latest` alias per design doc:96-98. Supersedes `HANDOFF.md:53` / pre-reg:40's gpt-5.4 references (2026-07-14-era free-handshake narration, which the pre-reg itself marks "NOT the frozen pin") |
...
# post-#117 resolution (2026-07-19), both arms:
constancy_failures: ["pin_mismatch:config_sha256", "pin_mismatch:codex_mcp_list_sha256",
                      "pin_mismatch:world_image_id",
                      "pin_mismatch:host_code_sha256", "pin_mismatch:image_code_sha256"]

# current (2026-07-21, after David's gpt-5.6-sol model-pin decision), both arms:
constancy_failures: ["pin_mismatch:planned_model", "pin_mismatch:brain_config_sha256",
                      "pin_mismatch:config_sha256", "pin_mismatch:codex_mcp_list_sha256",
                      "pin_mismatch:world_image_id",
                      "pin_mismatch:host_code_sha256", "pin_mismatch:image_code_sha256"]
```

No `expected_missing:*` on either arm (all 20 fields present). `tool_schema_sha256` MATCHES the
real receipts. The seven current mismatches are all correct behavior, in two classes:

- **Genuine staleness of the 2026-07-14 receipts (5):** `world_image_id` -- those receipts pin the
  SUPERSEDED pre-rebuild images (`sha256:8701e664...` red, `sha256:7128be60...` miniwob), while the
  pin now holds PR #117's rebuilt digests; `host_code_sha256`/`image_code_sha256` -- those
  ...
  contain); and `planned_model`/`brain_config_sha256` -- those receipts were made in the gpt-5.4
  era, while David pinned the brain to `gpt-5.6-sol` on 2026-07-21 (and the brain config embeds
  the model, so its hash moved with it). FRESH free handshakes against the rebuilt images with
  `-Model gpt-5.6-sol` would match all five; the stale receipts failing is the pin doing its job.
...
Both files are frozen against commit `609ab8a16ea6a7606827d08ff3b5c137cc751104` (the batched-world-rebuild source
commit; supersedes the earlier `b45b47f3f77690f1678e5476d3ea7b95decd34e3` freeze point); `world_image_id`/`tool_schema_sha256`/`image_code_sha256` confirmations were
