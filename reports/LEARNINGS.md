...
- **goto(target) nav (feature #2, local half, free):** the autopilot can now A* to a *named cell* (reusing its BFS), not just the nearest frontier — so the planner can later say "go to the gym" and the free pathfinder executes (the LLM-sets-the-target hookup is a small addition, testable once credits land).
- **Honest blocker still:** LLM-side validation (battle decisions, the goto hookup, the live Brock slice) waits on Anthropic credits. Everything above is built + unit-tested for $0 and ready.
