...
_Disposition: this is a **report-only** bank of the finding. The full experimental code
(`core/tilemap.py` `fp_match`, the `perceiver.py` recovery-rebind path, and the added tests) lives on
branch `feat/f4-keystone-fingerprint` (PR #150) for reproducibility only — it is **NOT merged** and
must not be, because it regresses the perceiver (it introduces confident-wrong MERGEs that the
pre-fix recovery path could not produce). PR #150 stays open, unmerged, as the reproducibility
pointer; only this report lands on `main`._
...
is confidently WRONG, not merely a re-mint that is honestly conservative. Baseline (PR #135)
all-frames homogeneity was already <1.0 (0.982 seed 3 / 0.990 seed 7) from warp-lag/seamless-boundary
frames, so the all-frames MERGE set was non-empty pre-fix — MERGE as such was not structurally
impossible. What changed: the pre-fix RECOVERY path could not produce a confident-wrong re-bind (it
always minted fresh); this fix introduces SETTLED re-binds that collapse distinct maps, dropping
seed-3 homogeneity 0.982→0.903, below the pre-registered 0.95 floor. The keystone hypothesis — that a whole-frame
...
- **Verified fact**: full test suite green (1479/1479, 16 pre-existing skips) after the change; the
  ...
  unedited.
- **Verified fact**: the pre-fix RECOVERY path could not produce a confident-wrong re-bind (every
  recovery unconditionally minted fresh) — so the SETTLED-re-bind MERGE observed here is
  attributable to this change, not a pre-existing scorer artifact. (All-frames MERGE from
  warp-lag/seamless-boundary frames was already present in the PR #135 baseline — that is a
  different, already-known phenomenon, not what this result reports.)
