...
def main() -> int:
    ...
    for step in range(args.rooms):
        ...
        for i in range(args.trials):
            r = run(base, gen(seed, 1400))
            seed += 1
            if r["rooms"] and not r["died"]:
                tag = f"adv_{step:02d}"
                r = run(base, gen(seed - 1, 1400), settle=500, save_as=tag)
                print(f"room {step}: -> {tag}  score={r['score']} hp={r['hp']} "
                      f"lives={r['lives']} D052={r['d052']} candidates5={r['cand']}")
                ...
                break
        if not found:
            print(f"room {step}: no transition found in {args.trials} trials (from {base})")
            return 1
