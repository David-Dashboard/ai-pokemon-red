...
| **Primitive ossification** (the inverse: too *much* per-world code — a perceiver/infra duplicated instead of lifted to `core/`; INSIGHTS §2) | a 2nd world copies a sibling's emulator/plugin/perceiver instead of reusing a `core/` primitive | ✅ fitness test `test_lean_games_do_not_carry_their_own_infra` (no `emulator.py`/`plugin.py` outside `pokemon_red`); lean perceivers are thin config over `core/grid_perceiver`; **checklist: before copying a sibling game's file, STOP — a duplicate is the signal to lift the shared part to `core/`** |
