...
from core.patience import Patience
...
# How many auto-advanced-past dialog lines PATIENCE surfaces in the observe render (the newest ones;
# older lines are counted but omitted). Mirrors core/brains._TRANSCRIPT_CAP — cap the prompt, not the log.
_TRAIL_RENDER_CAP = 12
...
class PerceptionPlugin:
    ...
    def __init__(
        ...
        # PATIENCE (2026-07-02 design): auto-advance a plain gated-static screen (a decoder-backed
        # dialog/cutscene textbox) for free, so the brain's next observe() never lands mid-textbox.
        # On by default and SAFE by default (PR #49 S1): the default Patience trusts only the
        # decoder-backed labels (Red's dialog/battle_text); a generic world's motion-derived
        # "static"/"menu" labels classify as choice and the loop never fires there. A world with a
        # trustworthy gated-static signal opts in by passing patience=Patience(extra_gated_contexts=...).
        self.patience = patience if patience is not None else Patience()
        # Set True by a driver that keeps per-step PNGs as logs (world_mcp --keep-frames): patience's
        # intermediate frames then get unique paths instead of overwriting one temp path.
    ...
    def observe(self, agent_id: str) -> Observation:
        ...
        # PATIENCE: if the frame is gated-static (a plain textbox/cutscene waiting for an advance
        # input — never a choice), auto-advance it FOR FREE right here, before returning to the brain,
        # so the caller's next observe() lands on free-control or a real choice. Re-perceives after
        # each press (settle-to-stable IS the re-perceive: the perceiver only reports free control
        # once the screen has actually changed), so a slow-fading textbox is never skipped past.
        advanced = 0
        trail: list[dict] = []   # per-press audit trail: what was advanced past, with which button
        self.patience.note_state_class(self.patience.classify(sym.context))
        if self.patience.classify(sym.context) == "gated-static":
            def _press_and_reperceive(button: str):
                nonlocal sym, screen_path
                prev_context, prev_text = sym.context, sym.screen_text
                frame_before = self.emu.frame
                try:
                    prev_pixels = self.emu.screen_ndarray()
                except Exception:
                    prev_pixels = None
                # S4 traceability: record the frame being advanced PAST (its decoded text) + the button.
                trail.append({"button": button, "context": prev_context, "text": prev_text})
                self.emu.press(button, hold_frames=8)
                self._sample_fade()
                self._last_action = button   # keep ctx["last_action"] honest for this internal press
                # Unique PNGs for intermediate frames only when the driver keeps frames as logs
                # (world_mcp --keep-frames sets plugin.keep_frames); otherwise reuse one path so the
                # loop doesn't accumulate frame debris the driver never sees to delete.
                suffix = f"_p{len(trail):02d}" if self.keep_frames else ""
                sym, screen_path = self._perceive_once(suffix=suffix)
                ...
                # free-control). Fall back to a raw pixel diff for text-less gated-static screens (an
                # opted-in world with no text decoder): any world can be control-grounded this way.
                # frames_advanced==0 gates it all (the #44 frozen-frame lesson): if the emulator's frame
                # counter didn't move, the screen could not have changed — never credit the button.
                ...
                if not changed and prev_pixels is not None:
                    try:
                        ...
                        pass
                if self.emu.frame - frame_before <= 0:
                    changed = False
        ...
        self._log_oracle(screen_path, sym, advanced=advanced, trail=trail)
        data = sym.to_dict()
        ...
        text = self._render_symbolic(sym)
        # S4: the advanced-past dialog content must reach the brain (the #41 render intent) — surface
        # the skipped lines (deduped consecutively; a held frame repeats its line between presses) in
        # the SAME observe's render, capped so a long chain can't bloat the prompt.
        skipped = []
        for t in trail:
            if t["text"] and (not skipped or skipped[-1] != t["text"]):
                skipped.append(t["text"])
        if skipped:
            shown = skipped[-_TRAIL_RENDER_CAP:]
            omitted = len(skipped) - len(shown)
            head = f"[auto-advanced past {advanced} frame(s)"
            if omitted > 0:
                head += f"; first {omitted} line(s) omitted"
            text += "\n" + head + ": " + " / ".join(f'"{s}"' for s in shown) + "]"
        return Observation(data=data, text=text, agent_id=agent_id, t=time.time())

    def _perceive_once(self, suffix: str = ""):
        ...
        screen_path = os.path.join(self.out_dir, f"frame_{self._obs_count:06d}{suffix}.png")
        try:
    ...
    def _log_oracle(self, screen_path: str, sym, advanced: int = 0,
        ...
        if trail:
            # S4 audit trail: WHAT was auto-advanced past (per-press button + context + decoded text),
            # not just how many times. Compact list on the observe record, one entry per free press —
            # also distinguishes auto presses from brain-commanded ones (these have a `button` here;
            # the brain's actions land in `perceived.action` via _post_action).
            rec["patience_trail"] = trail
        if self._watch:
            try:
    ...
    def _post_action(self, call: ToolCall, action: str) -> ToolResult:
        ...
        # PATIENCE budget (S2): a brain-commanded action DIFFERENT from its previous one re-arms the
        # per-episode auto-advance budget (the brain is trying something new); repeating the same
        # action on the same stuck gated screen does not.
        self.patience.note_brain_action(action)
